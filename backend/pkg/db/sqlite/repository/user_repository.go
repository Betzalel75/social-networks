...
func CreateUser(db *sql.DB, user model.User) error {
	stmt, err := db.Prepare("INSERT INTO users (user_id, username, email, password, photo, age, gender, firstName, lastName,keys, status_profile) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?,?,?);")
	if err != nil {
		tools.Log(err)
		return err
	}
...
func GetUserByEmail(db *sql.DB, email string) (model.User, error) {
	querySQL := `SELECT user_id, username, password, photo, age, gender, firstName, lastName,keys FROM users WHERE email = ?;`
	var u model.User
...
func GetUserByNickName(db *sql.DB, username string) (model.User, error) {
	querySQL := `SELECT user_id, email, password, photo, age, gender, firstName, lastName,keys FROM users WHERE username = ?;`
	var u model.User
