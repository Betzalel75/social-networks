...
func getGroupMessage(w http.ResponseWriter, groupID string) {
	...
	_, err := repo.GetGroupByGroupID(bd.GetDB(), groupID)
	if err != nil {
		tools.Log(err)
		return
	}
...
func newViewNotification(w http.ResponseWriter, r *http.Request, wsServer *WsServer) {
	var datas model.MessageData
	if err := json.NewDecoder(r.Body).Decode(&datas); err != nil {
		tools.Log(err)
		data := map[string]interface{}{
...
func responseForInvit(w http.ResponseWriter, r *http.Request, wsServer *WsServer) {
	...
	// Calculer l'offset
	notificationsPerPage := 2
	notifications, _ := repo.GetNotificationByUser(bd.GetDB(), userID, 0, notificationsPerPage)
	usr := make([]map[string]interface{}, 0)
	groupName := ""
